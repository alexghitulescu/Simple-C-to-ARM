We use the Parsec library to parse a small grammar of expressions and statements. The main purpose is to showcase <hask>makeTokenParser</hask> and <hask>buildExpressionParser</hask>, which cover a large class of common applications.

(Parsec comes with [http://hackage.haskell.org/platform/ Haskell Platform]; also available as [http://hackage.haskell.org/package/parsec/ parsec].)
